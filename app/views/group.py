...
class AllGroupView(MethodView):
    ...
    def get(self, course_code):
        ...
        result = self.control.get_all_groups(course_code)
        return json.dumps(result)
...
class GroupStaffView(MethodView):
    ...
    def post(self, course_code):
        logging.info("New POST /course/<string:course_code>/group/staffs request")
        ...
        if not data.get("group_type"):
            return json.dumps(Error.GROUP_TYPE_NOT_FOUND)
        result = self.control.create_group_staff(course_code, current_user, **data)
        return json.dumps(result)
...
app.add_url_rule('/course/<string:course_code>/group', view_func=IndividualGroupView.as_view('individual_group'),
                 methods=['GET'])
app.add_url_rule('/course/<string:course_code>/groups', view_func=AllGroupView.as_view('all_group'))
